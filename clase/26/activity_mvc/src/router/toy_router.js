...
router.post("/", async (req, res) => {
    const { name, description, creator_user } = req.body;

    if (!name || !description || !creator_user) {
        // Responder con un error si faltan campos obligatorios
        return res.status(400).send({ status: "error", message: "Faltan campos obligatorios: 'name', 'description' y 'creator_user'" });
    }

    try {
        const result = await toyController.createToy({ name, description, creator_user });
        res.send({ status: "success", payload: result });
    } catch (error) {
        res.status(500).send({ status: "error", message: error.message });
    }
